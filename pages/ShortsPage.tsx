...
const ShortsPage: React.FC = () => {
    ...
    const handleNext = useCallback(() => {
        ...
        if (nextIndex < videos.length) {
            ...
            if (nextVideo) {
                ...
                if (nextIframe) sendCommand(nextIframe, 'playVideo');
            }
    ...
    const handlePrev = useCallback(() => {
        ...
        if (prevIndex >= 0) {
            ...
            if (prevVideo) {
                ...
                if (prevIframe) sendCommand(prevIframe, 'playVideo');
            }
    ...
    if (isLoading && videos.length === 0) return <div className="flex justify-center items-center h-[calc(100vh-3.5rem)]"><div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-yt-blue"></div></div>;
                     ...
                     {videos.map((video, index) => {
                         ...
                         return (
                             <div 
                                ...
                                style={{ visibility: isActive ? 'visible' : 'hidden', zIndex: isActive ? 2 : 1 }}
                             >
                                 <ShortsPlayer 
                                    ref={(el) => {
                                    ...
                                    onLoad={(e) => {
                                        ...
                                        if (index === currentIndex && isShortsAutoplayEnabled) sendCommand(iframe, 'playVideo');
                                        else sendCommand(iframe, 'pauseVideo');
                                    }}
                                 />
                             </div>
                         );
