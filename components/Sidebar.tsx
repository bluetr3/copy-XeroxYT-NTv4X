...
const SidebarItem: React.FC<{ icon: React.ReactNode; label: string; to: string; onClick?: (e: React.MouseEvent) => void }> = ({ icon, label, to, onClick }) => (
    <NavLink 
      to={to} 
      end={to === '/'}
      onClick={onClick}
      className={({ isActive }) => 
        `flex items-center px-3 py-2.5 mx-3 rounded-lg text-[15px] transition-colors ${isActive && !onClick ? 'bg-[#f2f2f2] dark:bg-[#272727] font-semibold' : 'hover:bg-[#f2f2f2] dark:hover:bg-[#272727] font-normal'}`
      }
    >
  ...
  if (!isOpen) {
    return (
        <div className={`fixed top-14 left-0 w-[72px] h-full ${smallSidebarBgClass} flex-col items-center px-1 space-y-0 z-40 hidden md:flex transition-colors duration-300`}>
            ...
            <SmallSidebarItem to="/history" icon={<HistoryIcon />} label="履歴" />
        </div>
    );
  }
  ...
  return (
    <aside className={`fixed top-14 left-0 w-60 h-full ${sidebarBgClass} pb-3 hover:overflow-y-auto overflow-y-hidden z-40 group hidden md:block transition-colors duration-300`}>
      ...
      <nav className="flex flex-col space-y-0.5">
        ...
        <SidebarItem to="/you" icon={<PlaylistIcon />} label="マイページ" />
      </nav>
